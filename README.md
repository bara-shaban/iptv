...
The watch page shows `Cast` and `PiP` controls when direct source mode is active. `Cast` uses Safari AirPlay when available, then the browser Remote Playback API, then Picture in Picture as a fallback. VidAPI iframe mode cannot be cast as a direct video stream from this app; use browser-level `Cast tab` for iframes.
