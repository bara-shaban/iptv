(() => {
  ...
  async function setPreferredCurrentItem(item, fallbackUrl = buildPlayerUrl(item)) {
    ...
    setCurrentItem(item, fallbackUrl, { mode: "iframe" });
  }
  ...
  function syncCastButtons() {
    const hasDirectVideo = session?.sourceMode === "direct" && Boolean(els.video?.src);
  ...
  async function castCurrentVideo() {
    if (session?.sourceMode !== "direct" || !els.video?.src) {
  ...
  function buildStreamResolverUrl(item, resolverBase) {
    ...
    const url = new URL(resolverBase, window.location.href);
  ...
  function proxifyDirectUrlIfNeeded(sourceUrl) {
    ...
    try {
      ...
      const proxied = new URL("/__streamline-proxy", window.location.href);
